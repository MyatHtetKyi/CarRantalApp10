...
namespace CurRentaaApp
{
    public partial class ManageVehicleListing : Form
    {
        ...
        private void gvVehicleList_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                order = "a";
                gvVehicleList.DataSource = _db.TypeOfCars.Select(s => new { ID = s.Id, Make = s.Make, Model = s.Model, VIN = s.VIN, LicensePlateNumber = s.LicensePlateNumber }).OrderBy(s => s.ID).ToList();
            }
            else
            {
                order = "d";
                gvVehicleList.DataSource = _db.TypeOfCars.Select(s => new { ID = s.Id, Make = s.Make, Model = s.Model, VIN = s.VIN, LicensePlateNumber = s.LicensePlateNumber }).OrderByDescending(s => s.ID).ToList();
            }
        }
    }
}
